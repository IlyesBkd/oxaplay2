  ...
  return (
    <main className="w-full min-h-screen bg-zinc-950 text-white overflow-x-hidden">
      ...
      <section className="relative w-full flex flex-col lg:flex-row lg:min-h-[calc(100vh-72px)]">
        ...
        <div className="w-full lg:w-1/2 lg:sticky lg:top-[72px] lg:h-[calc(100vh-72px)] flex items-center bg-zinc-950">
          <div className="w-full max-w-xl mx-auto px-6 sm:px-10 lg:px-14 py-12 lg:py-0">
            <p className="text-[11px] font-medium text-zinc-500 uppercase tracking-[0.25em] mb-6">{t('CarplayVoiture.edition')}</p>

            <h1 className="text-3xl sm:text-4xl lg:text-5xl font-bold text-white mb-5 tracking-tight leading-[1.1]" dangerouslySetInnerHTML={{ __html: t('CarplayVoiture.title') }} />

            <p className="text-zinc-500 text-sm sm:text-base leading-relaxed mb-8 max-w-md">
              {t('CarplayVoiture.subtitle')}
            </p>

            {/* Rating */}
            <div className="flex items-center gap-3 mb-8">
              <RatingBadge />
              <span className="text-sm text-zinc-500">{t('CarplayVoiture.reviews')}</span>
            </div>
            ...
            <div id="commander" className="space-y-3 mb-8">
              ...
              >
                {t('CarplayVoiture.buyNow')}
                <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M13 7l5 5m0 0l-5 5m5-5H6" />
                </svg>
              </button>
              <p className="text-center text-xs text-zinc-600">{t('CarplayVoiture.deliverySecure')}</p>
            </div>
      ...
      <section className="w-full border-y border-zinc-800/50 bg-zinc-950 overflow-hidden">
        <div className="py-8">
          <p className="text-center text-[11px] font-medium text-zinc-500 uppercase tracking-[0.25em] mb-6">{t('CarplayVoiture.brandMarquee')}</p>
        ...
        <div className="max-w-7xl mx-auto px-6 py-12 sm:py-28">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-10 sm:gap-16 items-center">
            ...
            <div className="border-l border-zinc-800 pl-8">
              <p className="text-[11px] font-medium text-zinc-500 uppercase tracking-[0.25em] mb-5">{t('CarplayVoiture.focus.badge')}</p>
              <h2 className="text-3xl sm:text-4xl font-bold text-white leading-tight mb-8 tracking-tight" dangerouslySetInnerHTML={{ __html: t('CarplayVoiture.focus.title') }} />
              <p className="text-zinc-500 text-base leading-relaxed mb-6">
                {t('CarplayVoiture.focus.desc1')}
              </p>
              <p className="text-zinc-500 text-base leading-relaxed">
                {t('CarplayVoiture.focus.desc2')}
              </p>
            </div>
      ...
      <section className="w-full bg-zinc-950 py-12 sm:py-28">
        <div className="max-w-7xl mx-auto px-6">
          <p className="text-center text-[11px] font-medium text-zinc-500 uppercase tracking-[0.25em] mb-4">{t('CarplayVoiture.features.title')}</p>
          <h2 className="text-3xl sm:text-4xl font-bold text-center text-white mb-10 sm:mb-16 tracking-tight">
            {t('CarplayVoiture.features.subtitle')}
          </h2>

          <div className="grid grid-cols-2 gap-3 sm:gap-4">
            {(t.raw('CarplayVoiture.features.items') as any[]).map((f: any) => (
              <div
                key={f.title}
      ...
      <section className="w-full bg-zinc-950">
        <div className="max-w-7xl mx-auto px-6 py-12 sm:py-28">
          <p className="text-center text-[11px] font-medium text-zinc-500 uppercase tracking-[0.25em] mb-4">{t('CarplayVoiture.details.title')}</p>
          <h2 className="text-3xl sm:text-4xl font-bold text-center text-white mb-10 sm:mb-16 tracking-tight">
            {t('CarplayVoiture.details.subtitle')}
          </h2>
          ...
          <div className="grid lg:grid-cols-2 gap-6">
            ...
            <div className="rounded-2xl bg-zinc-900/50 border border-zinc-800 overflow-hidden">
              <div className="px-6 py-4 border-b border-zinc-800">
                <h3 className="text-[11px] font-medium text-zinc-400 uppercase tracking-[0.2em]">{t('CarplayVoiture.specs.title')}</h3>
              </div>
              {(t.raw('CarplayVoiture.specs.items') as any[]).map((s: any, i: number) => (
                <div
                  key={s.label}
                  className={`flex items-center justify-between px-6 py-3.5 ${i < (t.raw('CarplayVoiture.specs.items') as any[]).length - 1 ? "border-b border-zinc-800/50" : ""}`}
                >
            ...
            <div className="rounded-2xl bg-zinc-900/50 border border-zinc-800 overflow-hidden">
              <div className="px-6 py-4 border-b border-zinc-800">
                <h3 className="text-[11px] font-medium text-zinc-400 uppercase tracking-[0.2em]">{t('CarplayVoiture.box.title')}</h3>
              </div>
              <div className="px-6 py-4">
                <ul className="space-y-2">
                  {(t.raw('CarplayVoiture.box.items') as string[]).map((item: string, i: number) => (
                    <li key={i} className="text-sm text-zinc-300 flex items-start gap-2">
                      <span className="text-zinc-500">{item}</span>
                    </li>
                  ))}
                </ul>
                <p className="mt-4 pt-4 border-t border-zinc-800/50 text-xs text-zinc-600">
                  {t('CarplayVoiture.box.note')}
                </p>
              </div>
            </div>
          </div>
      ...
      <section className="w-full bg-zinc-950 py-12 sm:py-28">
        <div className="max-w-3xl mx-auto px-6">
          <p className="text-center text-[11px] font-medium text-zinc-500 uppercase tracking-[0.25em] mb-4">{t('CarplayVoiture.faq.badge')}</p>
          <h2 className="text-3xl sm:text-4xl font-bold text-center text-white mb-10 sm:mb-14 tracking-tight">
            {t('CarplayVoiture.faq.title')}
          </h2>

          <div className="space-y-3">
            {(t.raw('CarplayVoiture.faq.items') as any[]).map((item: any, i: number) => (
              <div
                key={i}
